## TALLER INTEGRADOR INDIVIDUAL 
# Daniel Maldonado Cantillo

| Defectos | Porqué era un problema | Como lo corrigio |
| --- | --- | --- |
| Nombre del archivo `Mi Pagina de Notas.HTML` | El nombre no sigue la convención de los nombres de los archivos | Renombre el archivo a `index.html` |
| IDs de inputs `n1`, `n2`, `n3` | Dificultan relacionar el HTML con la lógica que los usa | Se renombraron a `nota1`, `nota2`, `nota3` |
| IDs de párrafos de salida `r`, `r2` | No indican qué contenido muestran | Se renombraron a `parrafoPromedio` y `parrafoResultado` (reflejado también en `styles.css`) |
| Título de la página `pagina` | No describe el propósito del sitio | Se cambió a `Calculadora de Promedio` |
| Función comentada y sin uso `calcularAntiguo()` | Código muerto que ensucia el archivo | Se eliminó |
| Variable llamada `data1` | La variable no tiene proposito | Se eliminó |
| Variables llamadas `TempValue2`, `x` | No indican qué almacenan; obliga a leer todo el código para entenderlo | Se renombraron a `promedio`, `numeroNotas` |
| Parámetros llamados `a`, `b`, `c` | Mismo problema de legibilidad que las variables genéricas | Se renombraron a `nota1`, `nota2`, `nota3` |
| Función llamada `calc()` | No comunica su propósito dentro del código | Se renombró a `calcularPromedio()` || Variables llamadas `data1`, `TempValue2`, `x` | No indican qué almacenan; obliga a leer todo el código para entenderlo | Se renombraron a `notas`, `promedio`, `numeroNotas` |
| Parámetros llamados `a`, `b`, `c` | Mismo problema de legibilidad que las variables genéricas | Se renombraron a `nota1`, `nota2`, `nota3` |

URL DE LA PAGINA: 
